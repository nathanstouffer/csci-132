...
class IMP implements MouseListener {
    ...
    // current top value in stack
    PixelRegion current_top = null;
    ...
    IMP() {
        ...
        start.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent evt) {
                current_top = new PixelRegion(null, colorX, colorY);
                fun2();
                picture[colorY][colorX] = getPixels( new int[] { 255, 255, 255, 255 });
    ...
    * and then put the RGB back to an int (getPixels)
     */
    private void fun2() {
        int[] center_pixel_color = getPixelArray(picture[colorY][colorX]);
        while (current_top != null){
            while (current_top.getIndex() < 8){
                int[] center_pixel_loc = new int[] { current_top.getXCoor(), current_top.getYCoor() };
                int[] adjusters = current_top.getPixelAdjusters();
                int[] adjusted_pixel_color = getPixelArray(picture[center_pixel_loc[1] + adjusters[1]][center_pixel_loc[0] + adjusters[0]]);
                boolean close_color = true;
                for (int i = 0; i < 4 && close_color; i++){
                    if (Math.abs(center_pixel_color[i] - adjusted_pixel_color[i]) > 30){
                        close_color = false;
                    }
                }
                if (close_color){
                    picture[center_pixel_loc[1] + adjusters[1]][center_pixel_loc[0] + adjusters[0]] = getPixels( new int[] { 255, 255, 255, 255 });
                    current_top = new PixelRegion(current_top, center_pixel_loc[0] + adjusters[0], center_pixel_loc[1] + adjusters[1]);
                }
            }
            current_top = current_top.getPrevious();
        }
    }
    ...
    public static void main(String[] args) { IMP imp = new IMP(); }
